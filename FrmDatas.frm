...
Diaf = Str(Day(MskAte.Text)): Mesf = Str(Month(MskAte.Text)): Anof = Str(Year(MskAte.Text))
   ...
   If Trim$(MskDe.Text) > "" Then
      pcSelecao = "{tab_vendas.dta_venda}[1 to 4] + "
      pcSelecao = pcSelecao & " {tab_vendas.dta_venda}[6 to 7] +"
      pcSelecao = pcSelecao & " {tab_vendas.dta_venda}[9 to 10] >= '"
      pcSelecao = pcSelecao & Format(Anoi, "0000") _
                            & Format(Mesi, "00") _
                            & Format(Diai, "00") & "'"
      If Trim$(MskAte.Text) > "" Then
         pcSelecao = pcSelecao & " AND {tab_vendas.dta_venda}[1 to 4] + "
         pcSelecao = pcSelecao & " {tab_vendas.dta_venda}[6 to 7] +"
         pcSelecao = pcSelecao & " {tab_vendas.dta_venda}[9 to 10] <= '"
