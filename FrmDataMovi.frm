...
Diaf = Str(Day(MskAte.Text)): Mesf = Str(Month(MskAte.Text)): Anof = Str(Year(MskAte.Text))
   ...
   If Trim$(MskDe.Text) > "" Then
      pcSelecao = "{Tab_Movestoque.DaTA} >= Date("
      pcSelecao = pcSelecao & Format(Anoi, "0000") & "," & Format(Mesi, "00") & "," & Format(Diai, "00") & ")"

      If Trim$(MskAte.Text) > "" Then
         'pcSelecao = pcSelecao & " AND {tab_MovEstoque.data}[7 to 10] + "
         'pcSelecao = pcSelecao & " {tab_MovEstoque.data}[4 to 5] +"
         'pcSelecao = pcSelecao & " {tab_MovEstoque.data}[1 to 2] <= '"
         'pcSelecao = pcSelecao & Format(Anof, "0000") _
                               & Format(Mesf, "00") _
                               & Format(Diaf, "00") & "'"
         pcSelecao = pcSelecao & " AND {Tab_Movestoque.DaTA} <= Date("
   ...
   FrmCompras.CrRelcomp.Formulas(0) = "intervalo = " & "'" _
                      & " Movimentações no Periodo de " & MskDe.Text _
                      & " a " & MskAte.Text & "'"
     
   FrmCompras.CrRelcomp.Destination = 0 'Vídeo
   FrmCompras.CrRelcomp.WindowState = crptMaximized
   FrmCompras.CrRelcomp.WindowTitle = "Visualização das Movimentações do Período"
   'FrmCompras.CrRelcomp.SelectionFormula = pcSelecao
   FrmCompras.CrRelcomp.Formulas(1) = "nomeloja = '" & gNome & "'"
   'FrmCompras.CrRelcomp.SortFields(0) = "-{tab_vendas.codvend}"
